const getMainPage = require('./get-main-page');
const getLogsPage = require('./get-logs-page');

const getEditorPage = require('./get-editor-page');
const getProfilePage = require('./get-profile-page');
const getUtilitiesPage = require('./get-utilities-page');
const getAveragingPage = require('./get-averaging-page');
const getExperimentPage = require('./get-experiment-page');
const getMonitoringPage = require('./get-monitoring-page');
const getLiveTradingPage = require('./get-live-trading-page');
const getTradingDemoPage = require('./get-trading-demo-page');
const getTradesViewerPage = require('./get-trades-viewer-page');
const getTradingOnlinePage = require('./get-trading-online-page');
const getVolumeMonitoringPage = require('./get-volume-monitoring-page');
const getVolumeStatisticsPage = require('./get-volume-statistics-page');
const getRobotsStatisticsPage = require('./get-robots-statistics-page');
const getFigureLinesEditorPage = require('./get-figure-lines-editor-page');
const getFigureLevelsEditorPage = require('./get-figure-levels-editor-page');
const getMonitoringCorrelationPage = require('./get-monitoring-correlation-page');

const getLevelsMonitoringPage = require('./get-levels-monitoring-page');
const getFigureLinesMonitoringPage = require('./get-figure-lines-monitoring-page');

const getLoginPage = require('./get-login-page');
const getRegistrationPage = require('./get-registration-page');

module.exports = {
  getMainPage,
  getLogsPage,
  getProfilePage,
  getUtilitiesPage,
  getExperimentPage,
  getMonitoringPage,
  getLiveTradingPage,
  getTradingDemoPage,
  getTradesViewerPage,
  getTradingOnlinePage,

  getEditorPage,
  getAveragingPage,
  getVolumeMonitoringPage,
  getVolumeStatisticsPage,
  getRobotsStatisticsPage,
  getFigureLinesEditorPage,
  getFigureLevelsEditorPage,
  getMonitoringCorrelationPage,

  getLevelsMonitoringPage,
  getFigureLinesMonitoringPage,

  getLoginPage,
  getRegistrationPage,
};
